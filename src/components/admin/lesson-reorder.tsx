...
import { reorderArticleAction } from "@/app/[locale]/admin/articles/actions";
...
export function LessonReorder({
  ...
  const t = useTranslations("admin.modules.lessons.reorder");
  const router = useRouter();
  const [pending, startTransition] = useTransition();

  function move(direction: "up" | "down") {
    startTransition(async () => {
      const res = await reorderArticleAction({ id, direction });
      if (!res.ok) {
